...
if config_params["requestor_data_limiter"]:
    ...
    limiter_api_port = config_params["requestor_data_limiter_api_port"]
    rest_addr = f"127.0.0.1:{limiter_api_port}"
    central_net_addr = config_params["central_net_addr"]
    command_data_limiter = f".{os.path.sep}{data_limiter_executable} --listen-addr={listen_addr} --target-addr={central_net_addr} --rest-api-addr={rest_addr}"
    print(command_data_limiter)
    data_limiter_process = Popen(command_data_limiter, shell=True);
...
if data_limiter_process:
    data_limiter_process.communicate()
